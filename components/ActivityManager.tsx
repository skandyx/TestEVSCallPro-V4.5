...
const SYSTEM_IDS = ['activity-available', 'activity-disconnected'];
        ...
        <div className="fixed inset-0 bg-slate-800 bg-opacity-75 flex items-center justify-center p-4 z-50">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md">
                <div className="p-6 border-b dark:border-slate-700">
                    <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100">{activity ? "Modifier le Statut" : "Nouveau Statut"}</h3>
                </div>
                <div className="p-6 space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Nom du Statut</label>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Couleur de la LED</label>
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <header className="flex-shrink-0 mb-8">
                <h1 className="text-4xl font-bold text-slate-900 dark:text-slate-100 tracking-tight">Gestion des Statuts</h1>
                <p className="mt-2 text-lg text-slate-600 dark:text-slate-400">Gérer les types d'activités et statuts de pause pour les agents.</p>
            </header>
             ...
             <div className="flex-1 min-h-0 flex flex-col bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                <div className="flex-shrink-0 flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-semibold">Liste des Statuts</h2>
                    <button onClick={() => { setEditingActivity(null); setIsModalOpen(true); }} className="bg-primary hover:bg-primary-hover text-primary-text font-bold py-2 px-4 rounded-lg shadow-md inline-flex items-center">
                        <span className="material-symbols-outlined mr-2">add</span> Ajouter un Statut
                    </button>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        <thead className="bg-white dark:bg-slate-800 sticky top-0 z-10">
                            <tr>
                                ...
                                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Actif</th>
                         ...
                         <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                            {activityTypes.map(activity => {
                                const isSystem = SYSTEM_IDS.includes(activity.id);
                                return (
                                <tr key={activity.id}>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center gap-3">
                                            ...
                                            {isSystem && <span className="text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full">Système</span>}
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <ToggleSwitch 
                                            enabled={activeStates[activity.id] ?? true} 
                                            onChange={val => setActiveStates(s => ({...s, [activity.id]: val}))}
                                            disabled={isSystem}
                                        />
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                        <button onClick={() => { setEditingActivity(activity); setIsModalOpen(true); }} disabled={isSystem} className="text-link hover:underline inline-flex items-center disabled:text-slate-400 disabled:cursor-not-allowed">
                                            <span className="material-symbols-outlined text-base mr-1">edit</span>Modifier
                                        </button>
                                        <button onClick={() => handleDelete(activity.id)} disabled={isSystem} className="text-red-600 hover:text-red-900 dark:hover:text-red-400 inline-flex items-center disabled:text-slate-400 disabled:cursor-not-allowed">
                                            <span className="material-symbols-outlined text-base mr-1">delete</span>Supprimer
                                        </button>
                                    </td>
                                </tr>
                            )})}
